...
**Status**: **v1.1 FROZEN** (additive promotion from v1 FROZEN at commit `2affaa9`; codifies G-AT-PROD-1 retrospective lessons via Appendices C-F + §11.1. §1-§11 normative behavior unchanged from v1 — v1 clients are valid v1.1 clients without modification. Re-validated by 3-adapter re-conformance under G-AT-CONTRACT-V1.1: Claude Code 7/7 + OpenCode 7/7 + Codex 7/7 = 21/21 PASS, with OpenCode #02 inject_memory PASS-after-flaky-retry per the retry-once gate. Zero `_CONTRACT-BUG_`. Original v1 FROZEN status: validated by Claude Code spiral round 1 at commit `d04797e` and OpenCode spiral round 2 at commit `90500cb`; zero `_CONTRACT-BUG_` painpoints across both rounds — both rounds' painpoints classified as runtime-specific, addressed in adapter IMPLEMENTATION.md or upstream runtime fixes). Future contract changes require explicit Owner-authorized version bump and a new spiral round on at least one adapter.
**Frozen on**: 2nd adapter pass-through of conformance suite (v1, 2026-05-18). v1.1 ratified after 3-adapter re-conformance (also 2026-05-18).
...
- **v1.1 FROZEN (this) — 2026-05-18.** Additive promotion from v1 FROZEN. Adds Appendices C-F (Painpoint Classification / Worktree Isolation / Spiral Validation Minimums / Multi-Advisor Coordination) codifying G-AT-PROD-1 retrospective lessons. Adds §11.1 clarifying document-version vs `schema_version`-field axes. §1-§11 normative behavior unchanged from v1 — v1 clients valid as v1.1 clients without modification. **3-adapter re-conformance PASS** (Phase B iteration 2, evidence in `docs/proposals/v1.1-contract-impl-notes.md` at commits `c54a905` + `92f6f07`): Claude Code 7/7 + OpenCode 7/7 + Codex 7/7 = 21/21. OpenCode #02 inject_memory was first-attempt FAIL (`RUNTIME_FAILURE`, empty stdout); retry-once gate produced clean PASS — flakiness in OpenCode non-interactive mode, not a contract or runtime bug. Zero `_CONTRACT-BUG_`. Zero residual `_<RUNTIME>-BUG_` after retry.
